...
from server.server import _process_request
...
async def test_process_request_no_auth():
    headers = Headers()
    # Call the _process_request function to simulate a request to /login
    response = await _process_request("", headers)
...
async def test_process_request_wrong_path():
    ...
    response = await _process_request("/doest_exist", headers)
    assert response != None, "Should be rejects"
...
async def test_login_wrong_account():
    ...
    response = await _process_request("/login", headers)
...
async def test_signup():
    ...
    response = await _process_request("/signup", headers)
    assert response != None, "Should fail the login"
